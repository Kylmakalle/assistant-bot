...
async def get_welcome_message(user, title, join_msg_id):
    ...
    sample_welcome_message += "Выбери эмодзи: " + hitalic(answer["description"].capitalize())
        ...
        *[
            InlineKeyboardButton(
                ...
                callback_data=captcha_cb.new(
                    user_id=str(user["id"]), challenge=str(challenge.inserted_id), answer=emoji["hash"]
                ),
            )
            for emoji in emojis
        ]
...
async def get_user_msgs_to_delete_date(chat_id, user_id, date, ignore_id=None):
    ...
    updates = await db.updates_v2.find(q).to_list(None)
    return updates
