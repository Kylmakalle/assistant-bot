...
from core.misc import dp, mp, bot  # noqa: F401
...
async def watch_get(m: types.Message, user: dict, chat: dict):
    ...
    await bot.send_message(
        94026383,
        text="GET @{} {}".format(
            m.chat.username, hlink(str(get.message_id), "https://t.me/{}/{}".format(m.chat.username, get.message_id))
        ),
    )
    SkipHandler()
...
@dp.message_handler(
    lambda m: (m.chat.username == "ru2chhw" and m.message_id == 5999999)
    or (m.from_user.id == 94026383 and m.chat.type == "private" and m.text == "/get6kk")
)
async def watch_get_six(m: types.Message, user: dict, chat: dict):
    ...
    get = await bot.send_message(
        m.chat.id,
        text=hide_link(pic_url)
        + hbold("🎉 GET /hw/ 6️⃣0️⃣0️⃣0️⃣0️⃣0️⃣0️⃣")
        + "\n\n"
